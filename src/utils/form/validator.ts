...
const V = {
  ...
  req: (value: string | any[]) => makeResult(
    Array.isArray(value) ? value.length === 0 : value.trim() === '',
    'This field is required',
  ),

  min: (ln: number, checksOnlyTruthy = true) => (value: string) => makeResult(
    checksOnlyTruthy
  ...
  max: (ln: number) => (value: string) => makeResult(value.trim().length > ln, `Field must have ${ln} or less characters`),
  ...
  date: (checksOnlyTruthy = true) => (value: string) => makeResult(
    checksOnlyTruthy
  ...
  oneTruthy: (value: { [key: string]: boolean }) => makeResult(!Object.values(value).some((item) => !!item), 'At least one field must be checked'),

  sameAs: (idx: number, label: string) => (value: any, state: Form.State) => makeResult(value !== state.fields[idx].value, `Field must be same as ${label}`),

  unique: (values: string[]) => (value: string) => makeResult(
