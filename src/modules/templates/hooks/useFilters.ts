...
import {
  LIMIT, FILTERS, isValidCategory, TemplatesSearchFilters, TemplatesRouteProps,
} from '..';

const parseLimit = (limit: string) => (filters: TemplatesSearchFilters) => (!limit || isNaN(+limit) || +limit < LIMIT ? filters : { ...filters, limit });

const parsePage = (page: string) => (filters: TemplatesSearchFilters): TemplatesSearchFilters => (!page || isNaN(+page) ? filters : { ...filters, page });
